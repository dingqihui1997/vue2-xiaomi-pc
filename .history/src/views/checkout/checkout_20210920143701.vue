    ...
    <div class="backf5 box">
      <div class="containerAuto back3f container">
        ...
        <div class="miniprice flex-sba">
          ...
          <div>
            <div class="flex mat5">
              ...
              <div class="Themecolor flex1 textr">4件</div>
            </div>
            <div class="flex mat5">
              <div class="flex1">商品总价:</div>
              <div class="Themecolor flex1 textr">1146元</div>
            </div>
            <div class="flex mat5">
              <div class="flex1">优惠券抵扣:</div>
              <div class="Themecolor flex1 textr">-70元</div>
            </div>
            <div class="flex mat5">
              <div class="flex1">商品件数:</div>
              <div class="Themecolor flex1 textr">4件</div>
            ...
            <div class="flex-ja">
              应付金额:&nbsp;<span class="Themecolor font30">
                {{ price }}元</span
              >
            </div>
          </div>
        ...
        <div class="footer flex-sb">
          <div>
            <div class="flex">
              <div>{{ location ? location.username : "" }}</div>
              <div>{{ location ? location.mobile : "" }}</div>
            </div>
            <div>
              {{ location ? location.address + location.detailAddress : "" }}
            </div>
          </div>
          <div class="flex">
            <div class="back b0b0b0 flex-ja" @click="backcart">返回购物车</div>
            <div class="primary flex-ja color3f" @click="gotoOrder">
              立即下单
            </div>
          </div>
        </div>
      ...
      if (this.location) {
        this.$api
          .addOrder({
            user_id: JSON.parse(localStorage.getItem("user"))._id,
            price: this.price,
            address: this.location.address + this.location.detailAddress,
            count: this.goods.length,
            pay_time: Date.now(),
            goods_list: this.goods,
            mobile: this.location.mobile,
          })
          .then((res) => {
            console.log(res);
            if (res.code === 200) {
