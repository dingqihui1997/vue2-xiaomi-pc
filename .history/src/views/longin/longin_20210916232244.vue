        ...
        <div class="card">
          ...
          <div v-if="!active">
            <div class="formValidate-view" v-if="!show">
              ...
              >
                ...
                <FormItem>
                  <button
                    @click.prevent="handleSubmit('formValidate', formValidate)"
            ...
            <div v-if="show" class="formValidate-view">
              <Form ref="formMobile" :model="formMobile" :rules="ruleMobile">
                ...
                <FormItem prop="password">
                  <Input v-model="formMobile.code" placeholder="请输入验证码">
                    <template #append>
                      <div
                        @click="getLogincode"
                        :class="flag ? 'b0b0b0' : 'Themecolor'"
                      >
                        {{ flag ? `${num}秒后重新发送` : "发送验证码" }}
                      </div>
                    </template>
                  </Input>
                </FormItem>
                <FormItem>
                  <button
                    type="primary"
                    @click.prevent="handleSubmit('formMobile', formMobile)"
                    style="width: 100%; height: 60px"
                    :disabled="!(formMobile.mobile && formMobile.code)"
                    class="color3f"
                    :class="
                      formMobile.mobile && formMobile.code ? 'ipt1' : 'ipt'
                    "
                  >
                    登录
                  </button>
          ...
          <div v-if="active" class="formValidate-view">
            <Form ref="register" :model="register" :rules="ruleRegister">
              ...
              <FormItem prop="code">
                <Input v-model="register.code" placeholder="验证码">
                  <template #append>
                    <div
                      @click="getcode"
                      :class="flag && !value ? 'b0b0b0' : 'Themecolor'"
                    >
                      {{ flag && !value ? `${num}秒后重新发送` : "发送验证码" }}
                    </div>
                  </template></Input
                >
              </FormItem>
              <FormItem>
                <button
                  @click.prevent="registerSubmit('register', register)"
                  style="width: 100%; height: 60px"
                  :disabled="
                    !(
                      register.password &&
...
export default {
  ...
  methods: {
    // 登录
    handleSubmit(name, item) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          if (this.show) {
            //手机验证码登录
            this.$api.mobileLogin(item).then((res) => {
              this.success(res);
            });
          } else {
            //密码登录
            this.$api.userLogin(item).then((res) => {
              this.success(res);
            });
          }
    ...
    getcode() {
      this.value = 0;
      if (!this.timer && this.register.mobile) {
        this.openGetValidStr(this.register.mobile);
      } else {
        Message.error("填写有误请检查手机号");
      }
    },
    getLogincode() {
      this.value = 1;
      if (!this.timer && this.formMobile.mobile) {
        this.openGetValidStr(this.formMobile.mobile);
      } else {
        Message.error("填写有误请检查手机号");
      }
    },
